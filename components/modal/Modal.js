...
const Modal = ({ isShowing, hide, component, projectData = null }) => isShowing ? ReactDOM.createPortal(
  <>
    ...
    <div
      className="fixed inset-0 z-[1050] w-[100%] h-[100%] overflow-y-scroll outline-0"
      aria-modal
      aria-hidden
      tabIndex={-1}
      role="dialog"
    >
      ...
      >
        <div className="h-auto w-sreen overflow-hidden">
          <button
            type="button"
            className="absolute right-0 top-[0%] text-[2em] text-bold ml-[.3em] px-[0] py-[1rem] border-none cursor-pointer z-[1060]" data-dismiss="modal"
            aria-label="Close"
            onClick={hide}
          >
